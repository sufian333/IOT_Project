...
class RefrigeratorBOT(object):
	def __init__(self,TOKEN,img_TOKEN,CATALOG,WS_products_url):
		...
		self.keyboard2 = InlineKeyboardMarkup(inline_keyboard=[
					   ...
					   [InlineKeyboardButton(text='Humidity', callback_data='/humidity')],[InlineKeyboardButton(text='Total_Vaccine', callback_data='/vaccine')],[InlineKeyboardButton(text='Enable Tempeature Allarm', callback_data='/Vaccine_enable_allarm')],
					   [InlineKeyboardButton(text='Enable Tempeature Allarm', callback_data='/enable_allarm')],[InlineKeyboardButton(text='Disable Vaccine Allarm', callback_data='/Vaccine_disable_allarm')]
	...
	def on_chat_message(self,msg):
		...
		if chat_id in list(self.data.keys()):
			...
			self.bot.sendMessage(chat_id, "SmartVaccineRefrigerator: use the menu' to do what you want", reply_markup=self.keyboard)
	...
	def on_callback_query(self,msg):
		...
		if(query_data == '/register' or query_data == '/login'):
				...
				if ind==-1:
					...
					for j,sample in enumerate(feeds):
						if sample["field"+field] is not None:
							ind=j
				Temperature=str(json.loads(r.text)['feeds'][ind]['field'+field].encode('utf-8'))
				self.bot.sendMessage(chat_id, text='Total Vaccine is: ' + Vaccine + ' !')
				self.bot.sendMessage(chat_id, text="https://thingspeak.com/channels/"+chID+"/charts/"+field)
			except:
				self.bot.sendMessage(chat_id, text="Data not avaliable")
			...
			r=self.s[chat_id].put(self.CATALOG+'/newcontact',data=json.dumps({'telegramID':chat_id}))		
			if r.status_code == 200:
				self.bot.sendMessage(chat_id, text="Temperature Allarm enabled - Click on @RefrigeratorAllarm_bot to receive allarm")
			else:
		...
		elif(query_data == '/Vaccine_enable_allarm'):
			r=self.s[chat_id].put(self.CATALOG+'/newcontact',data=json.dumps({'telegramID':chat_id}))		
			if r.status_code == 200:
				self.bot.sendMessage(chat_id, text="Vaccine Allarm enabled - Click on @VaccineAllarambot to receive allarm")
			else:
				self.bot.sendMessage(chat_id, text="Error enabling allarm")
			
		elif(query_data == '/Vaccine_disable_allarm'):
			r=self.s[chat_id].put(self.CATALOG+'/newcontact',data=json.dumps({'telegramID':""}))		
			if r.status_code == 200:
				self.bot.sendMessage(chat_id, text="Allarm disabled")
			else:
